...
namespace System.Api.Api
{
    public class Startup
    {
        ...
        private void SecureApi(IServiceCollection services)
        {
            ...
            else
            {
                ...
                services.AddAuthorization(options =>
                {
                    options.AddPolicy("Write", policy =>
                    {
                        policy.RequireAssertion(context => context.User.HasClaim(clm => clm.Type == "scope" && clm.Value.Contains("AD_RW")));
                    });
                    options.AddPolicy("Read", policy =>
                    {
                        policy.RequireAssertion(context => context.User.HasClaim(clm => clm.Type == "scope" && (clm.Value.Contains("AD_RW") || clm.Value.Contains("AD_R"))));
                    });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            {
                options.SwaggerEndpoint("iis/swagger.json", "IIS Controller");
